...
import { convertUrlToFile, textFormatter } from "@/misc/utils";
...
const fetchCourses = async () => (rows.value = (await api("/courses/")).data);
const fetchCourse = async (id) => {
  let response = (await api(`/courses/${id}/`)).data;

  if (typeof response.image === "string") {
    const file = await convertUrlToFile(response.image, "image.png");
    if (file) response.image = file;
  }
...
const openEditForm = async (course) => {
  ...
  loadingItemId.value = course.id;
  selectedItem.value = await fetchCourse(course.id);
...
const confirmDelete = async () => {
  if (itemToDelete.value) {
    ...
    try {
      await api.delete(`/courses/${itemToDelete.value.id}/`);
      await fetchCourses();
      closeForm();
    } catch (error) {
      console.error(error);
